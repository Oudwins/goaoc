...
func sumDirSizes(tree *DirTree) (totalSize int, problemNodesSize int, problemNodes []string) {
	return recSumDirs(tree.root, &problemNodesSize, &problemNodes), problemNodesSize, problemNodes
}

func recSumDirs(node *DirNode, problemNodesSize *int, problemNodes *[]string) int {
	// base case
	if node == nil {
		return 0
	}
	...
	for _, dir := range node.childDirs {
		dirSize += recSumDirs(dir, problemNodesSize, problemNodes)
	}
	if dirSize < 100000 {
		// if dirSize < 4 {
